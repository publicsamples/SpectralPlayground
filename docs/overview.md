...
Build-wise, it should also remain independent from the HISE checkout. The repo is intended to carry its own JUCE dependency rather than inheriting one from a sibling project.
...
- source metadata
- analysis start offset within the source file
- FFT and hop settings
- frame times relative to the analyzed selection
- partial IDs
- partial frequencies
- partial amplitudes
